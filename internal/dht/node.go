...
// NewNode: creates a DHT node from bootstrap multiaddr list and bootstraps it
func NewNode(parent context.Context, namespace string, bootstrapAddrs []string) (*Node, error) {
	ctx, cancel := context.WithCancel(parent)

	var (
		h      host.Host
		dhtVal *dht.IpfsDHT // renamed from dht to dhtVal to avoid shadowing the import
		err    error
	)

	// --- Multi-interface IP detection and advertisement ---
	ips := detectIPv4Addrs()
	...
	opts = append(opts, libp2p.AddrsFactory(func(in []ma.Multiaddr) []ma.Multiaddr {
		...
		for _, a := range in {
			...
			if p, err := a.ValueForProtocol(ma.P_TCP); err == nil {
				// p is the port number as a string
				for _, ip := range ips {
			...
			// Other protocols: pass through as-is
		}
		return out
	}))
	...
	if len(bootstrapAddrs) > 0 { // wait only when bootstrap addresses are provided
...
func detectTailnetIPv4() string {
	...
	// If tailscale0 not found, search all interfaces for 100.64/10
	ifaces, _ = net.Interfaces()
	for _, iface := range ifaces {
	...
	for _, iface := range ifaces {
		...
		for _, a := range addrs {
			if ipnet, ok := a.(*net.IPNet); ok {
				...
				// Include only tailscale (100.64/10) or private network IPs
...
// Multiaddrs returns list of this peer's multiaddrs (+/p2p/ID)
